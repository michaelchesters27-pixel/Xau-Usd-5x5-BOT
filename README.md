...
3. When the open Buy basket is in drawdown, stale untriggered Sell Stops are cancelled and rebuilt immediately below the live price. When the open Sell basket is in drawdown, stale untriggered Buy Stops are rebuilt immediately above the live price. Open trades are never moved or replaced by this rule, and the EA never exceeds five active slots per side.
4. The whole campaign has a fixed **$5 combined profit target**. Combined profit means banked campaign profit plus current floating profit/loss, less estimated closing costs.
5. At $5, the EA closes every open campaign trade, deletes every untriggered order and begins a fresh 5×5 campaign.
6. The Railway dashboard accepts any positive **overall profit target** and **maximum loss**. These apply to the complete run across all campaigns.
7. When the next order on one side triggers, every earlier open order on that same side moves to breakeven:
   - Buy 2 protects Buy 1;
   - Buy 3 protects Buy 1 and Buy 2;
   - the rule continues through Buy 5 and is identical for sells.
8. Campaign protection arms after estimated closing costs and the $0.50 execution reserve are covered. If protected profit falls back to that safety floor, every open campaign trade closes and all remaining pending orders are deleted. A fresh campaign then begins.
9. When the overall profit target or maximum loss is reached, everything closes, all pending orders are deleted and the bot stops.
10. The dashboard has one trading control: **TURN BOT OFF**. It performs the same full close, delete and stop operation.
11. After OFF, remove and reattach the EA in MT5 to begin a new run. The same attached session cannot turn itself back on.
